...
- `voxcpm.rs` — Core VoxCPM model: config structs, `VoxCPM<B>` module with `base_lm`, `residual_lm`, `feat_encoder`, `feat_decoder` (UnifiedCFM), `fsq_layer`, projection layers (`enc_to_lm_proj`, `lm_to_dit_proj`, `res_to_dit_proj`, `fusion_concat_proj`), and stop head. The `fusion_concat_proj` layer (VoxCPM2) concatenates encoder outputs and masked feature embeddings along the last dimension before projecting back to hidden size, replacing the previous element-wise addition for residual LM input. Implements `generate()`, `generate_latent()`, `forward()` with autoregressive loop, and LibTorch-specific `generate_libtorch()` for bf16/f16 backends. Contains `VoxCPMLocDiT` (V1: mu+t concatenated as single token) and `VoxCPMLocDiTV2` (V2: mu and t as separate sequence tokens). `VoxCPMConfig` defaults updated for VoxCPM2: `patch_size=4`, `residual_lm_num_layers=8`, `scalar_quantization_latent_dim=512`, `max_length=8192`. Fields: `residual_lm_no_rope` (bool, default false), `ref_audio_start_token` (103), `ref_audio_end_token` (104), `audio_vae_config_v2: Option<AudioVaeConfigV2>` (present for voxcpm2 architecture). `VoxCPMDitConfig` has `dit_mean_mode` (bool, default false). Defines `AudioVAEBackend<B: Backend>` trait unifying V1 `AudioVae` and V2 `AudioVAEV2` decode interfaces (V2 requires `sr_cond` parameter). All `generate` methods use `&dyn AudioVAEBackend<AB>` instead of concrete `AudioVae`. Original Python reference: `OpenBMB/VoxCPM` dev_1.5 branch; V2 reference: `local_dit_v2.py`.
...
- `audiovae.rs` — Audio VAE V1 encoder/decoder: `AudioVaeConfig`, `CausalEncoder` (WN causal conv layers with strides [2,3,6,7,7]), `CausalDecoder` (transpose convs with residual units, dilations [1,3,9], Snake1d activation, optional NoiseBlock). Uses weight-normalized causal convolutions (`WNCausalConv1d`, `WNCausalTransposeConv1d`). Sample rate: 44100 Hz, out_sample_rate: 44100 Hz, latent dim: 64, chunk size: 1440, decode_chunk_size: 1440 (product of strides). Implements `AudioVAEBackend<B>` trait for unified V1/V2 interface. `decode()` converts to f32 via `.to_data().convert_dtype(DType::F32)` for generic backend support. Bias tensors are cast to input dtype at forward time to support mixed-precision (bf16/f16) inputs.
- `audio_vae_v2.rs` — Audio VAE V2 encoder/decoder for VoxCPM2 (48kHz output): `AudioVaeConfigV2`, `CausalEncoderV2` (strides [2,5,8,8]), `CausalDecoderV2` (strides [8,6,5,2,2,2], sample-rate conditioning via `SampleRateConditionLayer` with upstream default `sr_bin_boundaries=[20000,30000,40000]`). Uses causal left-pad WN convolutions (`WNCausalConv1dV2`, `WNCausalTransposeConv1dV2`). Input sample rate: 16000 Hz, output sample rate: 48000 Hz, latent dim: 64, hop_length: 640, decode_chunk_size: 1920. Implements `AudioVAEBackend<B>` trait for unified V1/V2 interface. Decoder pads latent channels when required, keeps AudioVAE decode in f32, applies compact sample-rate conditioning layers before decoder blocks, and casts conditioning embeddings/conv parameters to activation dtype to support mixed-precision (bf16/f16) inputs. Upstream reference: `audio_vae_v2.py`.
- `audio_utils.rs` — WAV file I/O utilities: `decode_wav_mono_f32()` (handles int/float, mono/stereo), `resample_mono_to_44100()` and `resample_mono_to_48000()` (linear interpolation via shared `resample_mono()`), `encode_wav_i16()`, `encode_pcm_i16()`, `decode_wav_bytes()`, `decode_wav_file()`.
